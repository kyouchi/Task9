...
std::vector<std::complex<double>> applyFft(std::vector<std::complex<double>>& input, const double sampling_rate,
   const double cutoff_frequency)
{
   ...
   const auto n_level = lc_fft_calc_ids(static_cast<int>(input.size()), &ids);// FFT事前計算
   lc_fft(input, ids, n_level, &output);// FFT変換
   return output;
}
