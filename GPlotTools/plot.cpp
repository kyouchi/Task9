...
bool Plot::printCommandFile()
{
   ...
   auto pngfile = filename_;
   const auto pos = pngfile.rfind(".");
   pngfile.erase(pos, pngfile.size());
   ...
   writing_command_file_ << "set term pngcairo enhanced size 1920,1080" << std::endl;
   writing_command_file_ << "set output \"" << pngfile << ".png\" " << std::endl;
   writing_command_file_ << "plot \"" << filename_ << "\" " << "with lp" << std::endl;
   writing_command_file_ << "set output" << std::endl;
